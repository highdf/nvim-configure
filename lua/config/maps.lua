...
-- [Lsp操作]
-----------------------------------------
map({ "n", "v" }, "<leader>la", "<cmd>lua vim.lsp.buf.code_action()<cr>", vim.tbl_extend('force', opt, {
	desc = "在当前光标处，弹出lsp命令菜单"
}))

-----------------------------------------
-- [其他操作]
-----------------------------------------
map("n", "<leader>gy", "<cmd>%y +<cr>", vim.tbl_extend('force', opt, {
	desc = "复制全部文本到系统剪切板"
}))
map("n", "<leader>gf", "<cmd>normal gg=G<cr>", vim.tbl_extend('force', opt, {
	desc = "格式化当前缓冲区"
}))
map("n", "<leader>gp", "<cmd>%d _ |put! +<cr>", vim.tbl_extend('force', opt, {
	desc = "用系统剪切板内容替换全部文本"
}))
...
map("i", "<C-'>", "()<c-[>i", vim.tbl_extend('force', opt, {
	desc = "插入()"
}))
map("i", "<C-l>", "<C-o>zt", vim.tbl_extend('force', opt, {
	desc = "清屏"
}))
