...
vim.o.scrolloff = 2           -- 垂直滚动时保留的上下文行数（光标距离窗口顶部/底部 2 行时开始滚动）
vim.o.sidescrolloff = 2       -- 水平滚动时保留的上下文列数（光标距离窗口左右边缘 2 列时开始滚动）
...
-- 缩进与制表符设置
-----------------------------------------
vim.opt.tabstop = 4           -- 显示 Tab 字符为 4 个空格宽度
vim.opt.shiftwidth = 4        -- 自动缩进/回退时的空格数（>> 和 << 命令）
vim.opt.expandtab = false     -- 保留 Tab 字符（不转换为空格），适合需要严格 Tab 的场景
...
-----------------------------------------
-- 注意事项：
-- 1. errorformat = "" 会完全禁用错误解析，建议仅在特殊场景使用
-- 2. expandtab=false 时建议搭配 :retab 命令维护文件一致性
-- 3. scrolloff 对 hjkl 和鼠标滚轮都生效，不只是 jk 移动
-----------------------------------------
