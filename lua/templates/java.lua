...
local function generatePackStat(relative_path)
	...
	if relative_path ~= "." then
		local package_path = relative_path:gsub(".*com", "com"):gsub("/", ".")
...
local function code(package_stat, classType, className, postSymbol, symbolName)
	return table.concat(
		{
			package_stat .. "\n",
			"\n",
...
local function dao_template(relative_path, filename)
	return code(
		generatePackStat(relative_path),
		"interface ",
		genenrateClassName(filename),
...
return function(opts)
	local template = {
		...
		{ pattern = ".*Dao%.java", content = dao_template },
